...
"""
    gammln(x::Float64) -> Float64

Returns the value ln[Gamma(x)] for x > 0.
Manual implementation from the original Fortran code.
"""
...
Returns rr array with dimensions corresponding to rr[1:2*lrm+1, 1:3, 1:lrm+1] where index [lrm+1, :, :] corresponds to n=0.
"""
function hermitian(yg::Float64, anpl::Float64, amu::Float64, lrm::Int, iwarm::Int)
    ...
    cr = -amu*amu/(constants.π_sqrt*cmxw)
    llm = min(3, lrm)
    bth2 = 2.0/amu
    bth = sqrt(bth2)
    amu2 = amu*amu
    ...
    # rr(0,1,2)
    rr[n_to_idx(0), 2, 3] = -2.0*anpl*bth4*(1.0 + bth2*(-1.5 + 1.5*anpl2) +
                           bth4*(39.0 - 69.0*anpl2 + 30.0*anpl4)/8.0)
    
    # rr(0,2,2)
    rr[n_to_idx(0), 3, 3] = -2.0*bth4*(1.0 + bth2*(0.75 + 1.5*anpl2) + bth4*
                           (13.0 - 48.0*anpl2 + 40.0*anpl4)*3.0/32.0)
    
    # rr(-1,0,2)
    rr[n_to_idx(-1), 1, 3] = -4.0*bth2/sy1*(1.0 + bth2*
                            (1.25 - 1.75/sy1 + 0.5*anpl2/sy1^2) + bth4*
                            (0.46875 - 3.28125/sy1 + (3.9375 + 1.5*anpl2)/sy1^2 -
                            3.375*anpl2/sy1^3 + 0.75*anpl4/sy1^4) + bth4*bth2*3.0/64.0*
                            (-5.0 - 35.0/sy1 + (210.0 + 40.0*anpl2)/sy1^2 - 3.0*(77.0 + 93.0*anpl2)/sy1^3 +
                            (396.0*anpl2 + 84.0*anpl4)/sy1^4 - 220.0*anpl4/sy1^5 + 40.0*anpl4*anpl2/sy1^6))
    
    # rr(-1,1,2)
    rr[n_to_idx(-1), 2, 3] = -2.0*bth4*anpl/sy1^2*(1.0 + bth2*
                            (3.0 - 4.5/sy1 + 1.5*anpl2/sy1^2) + bth4*
                            (20.0 - 93.0/sy1 + (99.0 + 42.0*anpl2)/sy1^2 -
                            88.0*anpl2/sy1^3 + 20.0*anpl4/sy1^4)*3.0/16.0)
    
    # rr(-1,2,2)
    rr[n_to_idx(-1), 3, 3] = -2.0*bth4/sy1*(1.0 + bth2*
                            (3.0 - 2.25/sy1 + 1.5*anpl2/sy1^2) + bth4*
                            (40.0*anpl4/sy1^4 - 132.0*anpl2/sy1^3 +
                            (66.0 + 84.0*anpl2)/sy1^2 - 93.0/sy1 + 40.0)*3.0/32.0)
    
    # rr(-2,0,2)
    rr[n_to_idx(-2), 1, 3] = -4.0*bth2/sy2*(1.0 + bth2*
                            (1.25 - 1.75/sy2 + 0.5*anpl2/sy2^2) + bth4*
                            (0.46875 - 3.28125/sy2 + (3.9375 + 1.5*anpl2)/sy2^2 -
                            3.375*anpl2/sy2^3 + 0.75*anpl4/sy2^4) + bth4*bth2*3.0/64.0*
                            (-5.0 - 35.0/sy2 + (210.0 + 40.0*anpl2)/sy2^2 - 3.0*(77.0 + 93.0*anpl2)/sy2^3 +
                            (396.0*anpl2 + 84.0*anpl4)/sy2^4 - 220.0*anpl4/sy2^5 + 40.0*anpl4*anpl2/sy2^6))
    
    # rr(-2,1,2)
    rr[n_to_idx(-2), 2, 3] = -2.0*bth4*anpl/sy2^2*(1.0 + bth2*
                            (3.0 - 4.5/sy2 + 1.5*anpl2/sy2^2) + bth4*
                            (20.0 - 93.0/sy2 + (99.0 + 42.0*anpl2)/sy2^2 -
                            88.0*anpl2/sy2^3 + 20.0*anpl4/sy2^4)*3.0/16.0)
    
    # rr(-2,2,2)
    rr[n_to_idx(-2), 3, 3] = -2.0*bth4/sy2*(1.0 + bth2*
                            (3.0 - 2.25/sy2 + 1.5*anpl2/sy2^2) + bth4*
                            (40.0*anpl4/sy2^4 - 132.0*anpl2/sy2^3 +
                            (66.0 + 84.0*anpl2)/sy2^2 - 93.0/sy2 + 40.0)*3.0/32.0)
    ...
    # Add terms for llm >= 3
    # rr(0,0,3)
    rr[n_to_idx(0), 1, 4] = -12.0*bth4*(1.0 + bth2*(0.75 + 0.5*anpl2) + bth4*
                           (1.21875 - 1.5*anpl2 + 0.75*anpl2*anpl2))
    
    # rr(0,1,3)
    rr[n_to_idx(0), 2, 4] = -6.0*anpl*bth6*(1 + bth2*(-0.25 + 1.5*anpl2))
    
    # rr(0,2,3)
    rr[n_to_idx(0), 3, 4] = -6.0*bth6*(1.0 + bth2*(2.5 + 1.5*anpl2))
    
    # rr(-1,0,3)
    rr[n_to_idx(-1), 1, 4] = -12.0*bth4/sy1*
                            (1.0 + bth2*(3.0 - 2.25/sy1 + 0.5*anpl2/sy1^2) +
                            bth4*(3.75 - 8.71875/sy1 + (6.1875 + 2.625*anpl2)/sy1^2 -
                            4.125*anpl2/sy1^3 + 0.75*anpl2*anpl2/sy1^4))
    
    # rr(-1,1,3)
    rr[n_to_idx(-1), 2, 4] = -6.0*anpl*bth6/sy1^2*
                            (1.0 + bth2*(5.25 - 5.5/sy1 + 1.5*anpl2/sy1^2))
    
    # rr(-1,2,3)
    rr[n_to_idx(-1), 3, 4] = -6.0*bth6/sy1*
                            (1.0 + bth2*(5.25 - 2.75/sy1 + 1.5*anpl2/sy1^2))
    
    # rr(-2,0,3)
    rr[n_to_idx(-2), 1, 4] = -12.0*bth4/sy2*
                            (1.0 + bth2*(3.0 - 2.25/sy2 + 0.5*anpl2/sy2^2) +
                            bth4*(3.75 - 8.71875/sy2 + (6.1875 + 2.625*anpl2)/sy2^2 -
                            4.125*anpl2/sy2^3 + 0.75*anpl2*anpl2/sy2^4))
    
    # rr(-2,1,3)
    rr[n_to_idx(-2), 2, 4] = -6.0*anpl*bth6/sy2^2*
                            (1.0 + bth2*(5.25 - 5.5/sy2 + 1.5*anpl2/sy2^2))
    
    # rr(-2,2,3)
    rr[n_to_idx(-2), 3, 4] = -6.0*bth6/sy2*
                            (1.0 + bth2*(5.25 - 2.75/sy2 + 1.5*anpl2/sy2^2))
    
    # rr(-3,0,3)
    rr[n_to_idx(-3), 1, 4] = -12.0*bth4/sy3*
                            (1.0 + bth2*(3.0 - 2.25/sy3 + 0.5*anpl2/sy3^2) +
                            bth4*(3.75 - 8.71875/sy3 + (6.1875 + 2.625*anpl2)/sy3^2 -
                            4.125*anpl2/sy3^3 + 0.75*anpl2*anpl2/sy3^4))
    
    # rr(-3,1,3)
    rr[n_to_idx(-3), 2, 4] = -6.0*anpl*bth6/sy3^2*
                            (1.0 + bth2*(5.25 - 5.5/sy3 + 1.5*anpl2/sy3^2))
    
    # rr(-3,2,3)
    rr[n_to_idx(-3), 3, 4] = -6.0*bth6/sy3*
                            (1.0 + bth2*(5.25 - 2.75/sy3 + 1.5*anpl2/sy3^2))
    ...
    # Add terms for llm >= 4
    # rr(0,0,4)
    rr[n_to_idx(0), 1, 5] = -48.0*bth6*(1.0 + bth2*(2.5 + 0.5*anpl2))
    
    # rr(0,1,4)
    rr[n_to_idx(0), 2, 5] = -24.0*anpl*bth8
    
    # rr(0,2,4)
    rr[n_to_idx(0), 3, 5] = -24.0*bth8
    
    # rr(-1,0,4)
    rr[n_to_idx(-1), 1, 5] = -48.0*bth6/sy1*(1.0 + bth2*(5.25 - 2.75/sy1 + 0.5*anpl2/sy1^2))
    
    # rr(-1,1,4)
    rr[n_to_idx(-1), 2, 5] = -24.0*anpl*bth8/sy1^2
    
    # rr(-1,2,4)
    rr[n_to_idx(-1), 3, 5] = -24.0*bth8/sy1
    
    # rr(-2,0,4)
    rr[n_to_idx(-2), 1, 5] = -48.0*bth6/sy2*(1.0 + bth2*(5.25 - 2.75/sy2 + 0.5*anpl2/sy2^2))
    
    # rr(-2,1,4)
    rr[n_to_idx(-2), 2, 5] = -24.0*anpl*bth8/sy2^2
    
    # rr(-2,2,4)
    rr[n_to_idx(-2), 3, 5] = -24.0*bth8/sy2
    
    # rr(-3,0,4)
    rr[n_to_idx(-3), 1, 5] = -48.0*bth6/sy3*(1.0 + bth2*(5.25 - 2.75/sy3 + 0.5*anpl2/sy3^2))
    
    # rr(-3,1,4)
    rr[n_to_idx(-3), 2, 5] = -24.0*anpl*bth8/sy3^2
    
    # rr(-3,2,4)
    rr[n_to_idx(-3), 3, 5] = -24.0*bth8/sy3
    
    # rr(-4,0,4)
    rr[n_to_idx(-4), 1, 5] = -48.0*bth6/sy4*(1.0 + bth2*(5.25 - 2.75/sy4 + 0.5*anpl2/sy4^2))
    
    # rr(-4,1,4)
    rr[n_to_idx(-4), 2, 5] = -24.0*anpl*bth8/sy4^2
    
    # rr(-4,2,4)
    rr[n_to_idx(-4), 3, 5] = -24.0*bth8/sy4
    ...
    for n in 1:lrm
        ...
        if rdu2 > 0.0
            ...
            if abs(aa) > 5.0  # if (abs(aa)>1.0_r8) then
                ...
                for m in n:lrm
                    cm = constants.π_sqrt * fact(m) * du^(2*m + 1)
                    cim = 0.5 * ci * dnl^m
                    mm = m - n + 1
...
function α(omega::Real, X::Real, Y::Real, N_r::Real, theta::Real, te:: Real, v_g_perp:: Real, imod:: Integer)
    N_par = N_r * cos(theta)
    Npr = sqrt(N_r^2 - N_par^2)
    mu = constants.m_e * constants.c^2/(te * constants.e)
    nharm = larmornumber(Y, N_par, mu)
    lrm = min(i_max, nharm)
    N_perp_cmplx = warmdisp(X, Y, N_par, mu, Npr, imod, 3, lrm)[1]
    N_warm = real(N_perp_cmplx) / sin(theta)
    return N_warm, 2.e0 * imag(N_perp_cmplx^2) * omega / constants.c * v_g_perp
end
